...
try:
  ...
  # ~~~~ INSERT CODE ~~~~
  socketServer.listen(6) # Make the server to start listening for connection requests from clients and the maximum queue
  # ~~~~ END CODE INSERT ~~~~
...
while True:
  ...
  try:
    # ~~~~ INSERT CODE ~~~~
    clientSocket, clientAddress = socketServer.accept() # The aim of this code is accept the client connection request
    # ~~~~ END CODE INSERT ~~~~
  ...
  # ~~~~ INSERT CODE ~~~~
  message_bytes = clientSocket.recv(BUFFER_SIZE)  # Accept the data from the client and read the most BUFFER_SIZE byte data
  # ~~~~ END CODE INSERT ~~~~
  ...
  try:
    ...
    # ~~~~ INSERT CODE ~~~~
    originServerSocket = socket.socket(socket.AF_INET, socket.SOCK_STREAM) # The aim is create a server of IPv4 and support TCP connect
    # ~~~~ END CODE INSERT ~~~~
    ...
    try:
      ...
      # ~~~~ INSERT CODE ~~~~
      originServerRequest = f"GET {resource} HTTP/1.1" # Use HTTP1.1 Get to make request and point to the resource and use HTTP1.1 protocol 
      ...
      # ~~~~ INSERT CODE ~~~~
      originServerR = originServerSocket.recv(BUFFER_SIZE) # Aim is make already connect client get the data, and use BUFFER_SIZE to limit the data.
      # ~~~~ END CODE INSERT ~~~~
      ...
      with open(cacheLocation, 'wb') as cacheFile:
        ...
        # ~~~~ INSERT CODE ~~~~
        cacheFile.write(originServerR) # Write the origin server response data in the aim cache file
        # ~~~~ END CODE INSERT ~~~~
